...
| Роль | Может | Не может |
|------|-------|----------|
| **Оператор** | Ввести текст, координаты, фото; проверить текст через LLM; скорректировать категорию и pileCount; отправить заявку | Публиковать на сайт, назначать `id`, видеть submissionId |
...
- сообщение об успешной отправке: **«Заявка отправлена администратору»** без submissionId
...
| **Frontend** | добавлены поля `category` (select) и `pileCount` (input) с бейджем AI |
| **Frontend** | автозаполнение полей после LLM-ответа |
| **Frontend** | исправлено зависание после ошибки LLM |
| **Frontend** | применена тема Sky Pro: бэджи, split-кнопка, разделитель AI-секции |
| **Frontend** | ymaps-статус: оранжевый `.pill`, inline-стили убраны из `app.js` |
| **Frontend** | сообщение об отправке: без submissionId, по центру |
| **Frontend** | убран hint `JPG/PNG → WebP…` под кнопкой фото |
| **Frontend** | AI-поля (категория, кол-во свай) и diff перенесены в отдельный `.ai-section` |
| **Docs** | обновлён `README.md` и `CONTEXT.md` под фактически реализованное состояние |
...
| 2026-05-28 | Применена тема Sky Pro: новый дизайн `index.html` и `styles.css` |
| 2026-05-28 | ymaps-статус оранжевый через CSS `.pill`; inline-стили убраны из `app.js` |
| 2026-05-28 | Сообщение об отправке: «Заявка отправлена администратору» без submissionId |
| 2026-05-28 | Убран hint под кнопкой фото; AI-секция и diff перенесены под разделитель |
| 2026-05-28 | Обновлён `README.md` и `CONTEXT.md` под финальное состояние сессии |
