...
- **Локальное приложение** на ПК (без публичного сервера на первом этапе)
...
- **Облако по API** (ключ в локальном конфиге, не в git)
...
- Модель по умолчанию: **Qwen 3.5 Flash** (проверена на домене сайта)
...
| Этап | Действие |
|------|----------|
| Оператор | Приём JPG/PNG/HEIC → **WebP без уменьшения разрешения** (запас под кадрирование) |
| Админ | Кадрирование, при необходимости ресайз при экспорте в прод |
...
- Ввод: **числа с валидацией** и/или **клик по карте** (Яндекс.Карты, как на сайте)
